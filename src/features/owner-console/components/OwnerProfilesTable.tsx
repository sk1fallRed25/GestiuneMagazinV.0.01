...
import { Users, Mail, User, Shield, CheckCircle2, XCircle, Calendar, Building2, UserPlus, Search, ChevronDown, ChevronUp, X } from 'lucide-react';
...
export const OwnerProfilesTable: React.FC<OwnerProfilesTableProps> = ({ profiles, loading, onOpenAssignModal }) => {
  ...
  const filteredProfiles = useMemo(() => {
    ...
    if (debouncedSearchTerm) {
      list = list.filter(p =>
        matchSearch([p.email, p.fullName, p.globalRole], debouncedSearchTerm)
      );
    }
    if (roleFilter !== 'all') {
  ...
  }, [profiles, debouncedSearchTerm, roleFilter, sortKey, sortAsc]);
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <div className="relative flex-1 flex items-center">
            ...
            <input
              ...
              className="w-full pl-9 pr-9 py-2 bg-white dark:bg-gray-700/50 border border-gray-200 dark:border-gray-700 rounded-xl text-xs font-medium text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
            />
            {searchTerm && (
              <button
                type="button"
                onClick={() => setSearchTerm('')}
                className="absolute right-3 top-1/2 -translate-y-1/2 p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full text-gray-400 hover:text-gray-200 transition-colors"
                        ...
                        <div className="min-w-0">
                          <div className="font-semibold text-gray-900 dark:text-white flex items-center gap-1.5 text-sm truncate">
                            ...
                            <span className="truncate">
                              <HighlightText text={profile.email} search={searchTerm} />
                            </span>
                          </div>
                          {profile.fullName && (
                            <div className="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1 mt-0.5">
                              ...
                              <span className="truncate">
                                <HighlightText text={profile.fullName} search={searchTerm} />
                              </span>
                            </div>
                          )}
                        </div>
