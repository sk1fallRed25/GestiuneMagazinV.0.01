...
export const dashboardService = {
    async getDashboardData(storeId: string): Promise<DashboardData> {
        ...
        // C. Toate produsele pentru health check & zero stock / price check
        ...
        const activeProducts = allProductsList.filter((p: any) => p.status === 'active');
        ...
        allProductsList.forEach((p: any) => {
            productStockMap[p.id] = {
                ...
                isSgr: p.sgr_enabled || false,
                sgrType: p.sgr_type || null
            };
        });
        ...
        allBatches.forEach(b => {
            ...
            // Populate stock levels in productStockMap
            const cachedProd = productStockMap[productId];
            if (cachedProd) {
                cachedProd.total += qty;
                if (zone === 'magazin') cachedProd.magazin += qty;
                else cachedProd.depozit += qty;
            }
        ...
        // D2. Toate produsele au fost mutate la începutul funcției.

        const productsWithoutPrice = allProductsList
        ...
        const productsWithZeroStock = allProductsList
            .filter((p: any) => p.status === 'active')
            .map((p: any) => {
        ...
        (salesHistory || []).forEach((s: any) => {
            ...
            items.forEach((item: any) => {
                ...
                // Exclude cancelled/voided sales since the query filters bystatus = 'finalized'

                // Calculate cost and profit
                const purchasePrices = item.stock_batches?.purchase_price;
                const pricePurchase = purchasePrices !== undefined && purchasePrices !== null
                    ? toNumberSafe(purchasePrices, 0)
                    : (item.products?.product_prices?.find((pp: any) => pp.store_id === storeId)?.price_purchase ?? 0);
                
                const cost = qty * pricePurchase;
                ...
                // Last sale tracking
                const currentLastSale = productLastSaleMap[productId];
                if (!currentLastSale || s.created_at > currentLastSale) {
                    productLastSaleMap[productId] = s.created_at;
                }
            });
        });

        // D5. Stocul a fost agregat la începutul funcției.
