...
export const PosCartEventsPanel: React.FC<PosCartEventsPanelProps> = ({ storeId }) => {
  ...
  return (
    <Card 
      ...
      className="p-6 border border-slate-300 shadow-sm flex flex-col gap-6"
    >
      <div className="flex flex-wrap justify-between items-center gap-4">
        ...
        <button 
          onClick={loadEvents} 
          disabled={loading}
          className="flex items-center gap-2 px-4 py-2 border border-slate-300 hover:bg-slate-50 text-slate-700 rounded-xl text-sm font-bold transition-all disabled:opacity-50"
        >
      ...
      <div className="flex flex-wrap gap-2 items-center text-xs font-semibold">
        <span className="text-slate-500 uppercase tracking-wider text-[10px]">Filtrează după acțiune:</span>
        <button 
          onClick={() => setFilterType('all')} 
          className={`px-3 py-1.5 rounded-lg border transition-all ${filterType === 'all' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-slate-600 border-slate-300 hover:bg-slate-50'}`}
        >
          Toate ({events.length})
        </button>
        <button 
          onClick={() => setFilterType('item_removed')} 
          className={`px-3 py-1.5 rounded-lg border transition-all ${filterType === 'item_removed' ? 'bg-red-600 text-white border-red-600' : 'bg-white text-red-600 border-slate-300 hover:bg-red-50'}`}
        >
          Ștergeri ({events.filter(e => e.event_type === 'item_removed').length})
        </button>
        <button 
          onClick={() => setFilterType('cart_cleared')} 
          className={`px-3 py-1.5 rounded-lg border transition-all ${filterType === 'cart_cleared' ? 'bg-rose-600 text-white border-rose-600' : 'bg-white text-rose-600 border-slate-300 hover:bg-rose-50'}`}
        >
          Goliri ({events.filter(e => e.event_type === 'cart_cleared').length})
        </button>
        <button 
          onClick={() => setFilterType('item_quantity_changed')} 
          className={`px-3 py-1.5 rounded-lg border transition-all ${filterType === 'item_quantity_changed' ? 'bg-amber-500 text-white border-amber-500' : 'bg-white text-amber-600 border-slate-300 hover:bg-amber-50'}`}
        >
      ...
      <div className="overflow-x-auto rounded-2xl border border-slate-300">
        <table className="w-full text-left text-sm text-slate-600 border-collapse">
          <thead>
            <tr className="bg-slate-100 text-[10px] font-black uppercase text-slate-700 tracking-wider border-b border-slate-300">
            ...
            {filteredEvents.length === 0 ? (
              ...
              filteredEvents.map(e => (
                ...
                >
                  ...
                  <td className="py-3.5 px-4 font-semibold text-slate-700 max-w-[120px] truncate" title={e.cashier_profile_id}>
                  ...
                  <td className="py-3.5 px-4" data-testid="pos-cart-event-product">
                    {e.product_name ? (
                      <div className="flex flex-col">
                        <span className="font-semibold text-slate-800">{e.product_name}</span>
                      ...
                      <span className="text-slate-400 italic" data-testid="pos-cart-event-quantity-change">
