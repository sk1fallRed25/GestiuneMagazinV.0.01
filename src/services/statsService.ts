...
export const fetchDashboardStats = async (): Promise<DashboardStats> => {
    ...
    const totalVanzariAzi = vanzariAziData?.reduce((sum, item) => sum + item.total, 0) || 0;

    const { data: produse } = await supabase.from('produse').select('stoc, prag_minim');
    const produseCritic = produse?.filter(p => p.stoc <= p.prag_minim).length || 0;
