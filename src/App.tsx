...
const StatCard = ({ title, value, icon: Icon, color, trend, loading }: any) => (
    <div className="bg-white rounded-2xl p-6 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] hover:shadow-xl transition-all duration-300 border border-gray-100 group relative overflow-hidden">
        ...
        {trend && (
            <div className="mt-4 flex items-center text-xs">
                <span className={`${trend.isPositive ? 'text-green-500' : 'text-red-500'} flex items-center font-bold`}>
...
// ==========================================
// LAYOUT PRINCIPAL (CU NOTIFICĂRI)
// ==========================================
...
const MainLayout = ({ children, onLogout, userRole }: { children: React.ReactNode, onLogout: () => void, userRole: string }) => {
    ...
    const fetchNotifications = useCallback(async () => {
        // Luăm produsele care sunt deja sub stocul minim
        const { data: produse } = await supabase
            .from('produse')
            .select('id, nume, stoc_depozit, stoc_minim_depozit')
            .lte('stoc_depozit', supabase.raw('stoc_minim_depozit')); // Doar cele critice (hack pt comparatie coloane: se face mai bine in JS filter)

        // Supabase query filters sunt limitate la comparatii cu valori fixe, asa ca filtram in JS pentru siguranta
        const { data: toateProdusele } = await supabase.from('produse').select('id, nume, stoc_depozit, stoc_minim_depozit');

        const critice = toateProdusele?.filter(p => p.stoc_depozit <= p.stoc_minim_depozit).map(p => ({
            id: p.id,
            type: 'alert',
    ...
    useEffect(() => {
        ...
        // ASCULTĂTOR REAL-TIME PENTRU STOCURI
        const subscription = supabase
            ...
            .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'produse' }, (payload) => {
                ...
                // Dacă stocul a scăzut sub minim și înainte era ok (sau a scăzut și mai mult)
                if (newProd.stoc_depozit <= newProd.stoc_minim_depozit) {

                    // Verificăm dacă nu cumva era deja critic (ca să nu spamăm, deși utilizatorul vrea update la orice schimbare critică)
                    // Dacă vrei notificare LA ORICE modificare când e critic:
                    const mesaj = `ATENȚIE: ${newProd.nume} a ajuns la stocul ${newProd.stoc_depozit}!`;

                    // Afișăm Toast (Pop-up)
                    toast.error(mesaj, { duration: 5000, position: 'top-right' });

                    // Adăugăm în lista de sus
                    setNotifications(prev => {
                        // Scoatem notificarea veche pt acelasi produs ca sa nu avem duplicate
                        const filtered = prev.filter(n => !n.message.includes(newProd.nume));
                        return [{
                            id: new Date().getTime(), // ID temporar unic
                            type: 'alert',
                            message: mesaj,
                            time: 'Chiar acum',
                            read: false
        ...
        <div className="flex h-screen bg-slate-50 font-sans overflow-hidden text-slate-800">
            ...
            <aside className="w-72 bg-[#0f172a] text-white flex flex-col shadow-2xl z-30 shrink-0 transition-all duration-300">
                    ...
                    {userRole === 'admin' && (
                        <>
                            ...
                            <div className="mx-2 mb-1">
                                <button
                                    onClick={() => setIsGestionarMenuOpen(!isGestionarMenuOpen)}
                                    className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition-all text-sm font-medium ${isGestionarMenuOpen ? 'bg-slate-800 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}
                                >
                                    <div className="flex items-center gap-3">
                                        <Briefcase size={18} />
                                        <span>Atribuții Gestionar</span>
                                    </div>
                                    <div className={`transition-transform duration-300 ${isGestionarMenuOpen ? 'rotate-180' : ''}`}>
                                        <ChevronDown size={16} />
                                    </div>
                                </button>
                                <div className={`grid transition-[grid-template-rows] duration-300 ease-in-out ${isGestionarMenuOpen ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                                    <div className="overflow-hidden bg-slate-900/50 rounded-b-xl mb-2">
                                        <div className="py-2 space-y-1">
                                            <LinkuriOperatiuni isSubmenu={true} />
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div className="p-4 bg-[#0a0f1c]">
                    <button onClick={onLogout} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-red-400 hover:bg-red-500/10 hover:text-red-300 transition-all text-sm font-medium border border-transparent hover:border-red-500/20">
                        <LogOut size={18} />
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="relative" ref={notifMenuRef}>
                            <button
                                ...
                                className={`p-2 rounded-full hover:bg-gray-100 relative transition-colors ${showNotifMenu ? 'bg-indigo-50 text-indigo-600' : 'text-gray-500'}`}
                            >
                                <Bell size={20} />
                            ...
                            {showNotifMenu && (
                                <div className="absolute right-0 top-12 w-80 bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                                    <div className="p-4 border-b border-gray-50 flex justify-between items-center bg-gray-50/50">
                                        ...
                                        {notifications.length > 0 && (
                                            <button onClick={() => setNotifications([])} className="text-xs text-indigo-600 hover:underline">Șterge tot</button>
                                        )}
                                    </div>
                                    <div className="max-h-64 overflow-y-auto">
                                        {notifications.length === 0 ? (
                                            <div className="p-8 text-center text-gray-400 text-sm">
                                                <div className="mx-auto w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center mb-2"><Bell size={16} /></div>
                        ...
                        <div className="flex items-center gap-3 pl-2">
                            <div className="text-right hidden md:block">
                                ...
                                <p className="text-xs text-gray-400">Magazin Central</p>
                            </div>
...
// --- DASHBOARD (Păstrat neschimbat, funcționează perfect) ---
const Dashboard = ({ userRole }: { userRole: string }) => {
    ...
    const fetchDashboardData = useCallback(async () => {
        try {
            ...
            const { data: vanzari } = await supabase.from('vanzari').select('total').gte('created_at', `${today}T00:00:00`).lte('created_at', `${today}T23:59:59`);
            const totalVanzari = vanzari?.reduce((acc, curr) => acc + curr.total, 0) || 0;

            setStats({ alerteStoc: alerteCount, comenziAgenti: comenziCount || 0, vanzariAstazi: totalVanzari, cereriFurnizori: cereriCount || 0, loading: false });
        } catch (error) { console.error("Eroare dashboard:", error); setStats(s => ({ ...s, loading: false })); }
    }, []);
    ...
    return (
        <div className="p-8 max-w-7xl mx-auto pb-20">
            <div className="mb-10"><h1 className="text-3xl font-bold text-gray-800 mb-2">Bine ai revenit! 👋</h1><p className="text-gray-500">Iată situația magazinului în timp real.</p></div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
                <Link to="/produse"><StatCard title="Alerte Stoc Critic" value={`${stats.alerteStoc} Produse`} icon={AlertTriangle} color={stats.alerteStoc > 0 ? "bg-red-500 text-red-600" : "bg-green-500 text-green-600"} trend={{ isPositive: stats.alerteStoc === 0, value: stats.alerteStoc > 0 ? "Necesită aprovizionare" : "Stoc Optim", label: "status curent" }} loading={stats.loading} /></Link>
                <StatCard title="Comenzi Agenți" value={`${stats.comenziAgenti} Noi`} icon={Inbox} color="bg-purple-500 text-purple-600" trend={{ isPositive: true, value: "În așteptare confirmare", label: "" }} loading={stats.loading} />
                <StatCard title="Vânzări Astăzi" value={`${stats.vanzariAstazi.toFixed(2)} RON`} icon={TrendingUp} color="bg-green-500 text-green-600" trend={{ isPositive: true, value: "Astăzi", label: "total încasări" }} loading={stats.loading} />
                {userRole === 'admin' && <StatCard title="Cereri Furnizori" value={`${stats.cereriFurnizori} În Așteptare`} icon={Briefcase} color="bg-blue-500 text-blue-600" loading={stats.loading} />}
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 bg-white rounded-2xl p-6 shadow-sm border border-gray-100 min-h-[300px]"><h3 className="font-bold text-lg text-gray-800 mb-4">Activitate Recentă</h3><div className="flex items-center justify-center h-full text-gray-400 text-sm bg-gray-50 rounded-xl border border-dashed border-gray-200">Grafic vânzări (În dezvoltare...)</div></div>
                <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100"><h3 className="font-bold text-lg text-gray-800 mb-4">Top Produse</h3><div className="space-y-4">{[1, 2, 3].map((_, i) => (<div key={i} className="flex items-center gap-3 p-3 hover:bg-gray-50 rounded-xl transition-colors cursor-pointer group"><div className="w-10 h-10 bg-indigo-100 text-indigo-600 rounded-lg flex items-center justify-center font-bold">#{i+1}</div><div><div className="h-4 w-24 bg-gray-200 rounded mb-1 group-hover:bg-gray-300 transition-colors"></div><div className="h-3 w-12 bg-gray-100 rounded"></div></div></div>))}</div></div>
            </div>
        </div>
    )
};
