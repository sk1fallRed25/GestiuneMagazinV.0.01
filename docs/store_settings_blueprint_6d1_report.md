# Store Settings Blueprint Report (Etapa 6D.1)

This report details the architectural audit and proposed design for operational settings in **Gestiune Magazin v2**, preparing the platform for local production pilot stages.
...
* **Custom VAT Rates**: Support for Romanian VAT tiers (19% standard, 9% food/drinks, 5% books/catering, 0% exempt).
...
1. **Schema Validation Trigger Function (`validate_store_settings_schema`)**: Evaluates new or updated JSONB payloads against typing and structure rules. Can be linked as a `CHECK` constraint.
2. **Access Helpers**:
   * `get_store_setting_text(store_id, path, default_value)`
   * `get_store_setting_numeric(store_id, path, default_value)`
...
## 4. Next Implementation Steps (Phase 6D.2)
...
3. **Replace Hardcoded Frontend Hooks**:
   * Map `useReception.ts` VAT selection options to `store.settings.tax.vat_rates`.
   * Map `useFastAdd.ts` VAT selection options to `store.settings.tax.vat_rates`.
