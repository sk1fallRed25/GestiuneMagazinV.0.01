...
> [!NOTE]
> Blueprint-ul SQL din acest raport a fost revizuit, securizat și întărit suplimentar (SQL Pre-Apply Hardening) în Etapa 6F.1.3 și 6F.1.3.1. Modificările aduse au inclus: validări automate ale dependențelor între module la activare/dezactivare în RPC, înlocuirea selectării oarbe cu Effective Access, definirea explicită a politicilor RLS (WITH CHECK) și restricționarea tuturor scrierilor exclusiv prin RPC (fără granturi directe DML pe tabele). În 6F.1.3.1 s-au adăugat validări de siguranță finale: blocarea explicită a activării modulelor planned (roadmap), verificarea obligativității lui `p_enabled`, normalizarea/validarea cheilor de module și validarea strictă a tipului boolean JSON în payload-ul bulk.
