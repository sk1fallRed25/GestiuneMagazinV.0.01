...
- **Etapa 5D.3**: Migrare Pierderi/Casări la RPC atomic `record_waste` (Realizat - frontend-ul nu mai execută pași multi-step pentru pierderi/casări. Transfer este deja validat operațional, iar Recepție/POS urmează să fie migrate).
- **Etapa 5D.4 - 5D.5**: Migrarea succesivă a celorlalte servicii frontend (`receptionService`, `posService`) pentru a apela noile funcții RPC. (Urmează 5D.4 Recepție / receive_stock)
