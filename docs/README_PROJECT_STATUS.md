...
- **Etapa 6B.3.2 (Sales Advanced Returns Frontend & Service Integration)**: Realizat (`docs/sales_returns_frontend_integration_6b32_report.md`). S-au integrat serviciile `getSaleReturnEligibility` și `returnSaleItems`, modalul `ReturnSaleModal`, butonul `RETUR PRODUSE` în detaliile bonului, validarea cantităților returnabile, calculul live al refundului și metodele de rambursare `cash/card/voucher`. Stocul rămâne modificat exclusiv prin RPC, iar E2E-ul urmează în 6B.3.3.
- **Următorul pas**: Etapa 6B.3.3 (Sales Advanced Returns E2E Test)
