...
- **Etapa 6B.2.1 (Următorul Pas)**: Manual SQL Apply + Verification — aplicarea manuală a scriptului `database/proposed_sales_void_mvp_6b2.sql` în consola Supabase și verificarea read-only a existenței tabelelor `sale_returns`, `sale_return_items`, a politicilor RLS și a granturilor pe procedurile stocate.
