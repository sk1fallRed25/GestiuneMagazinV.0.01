-- ############################################################################
-- BLUEPRINT SQL: Commercial Reports Upgrade (Etapa 6C.1)
-- Project: Gestiune Magazin v2
--
-- IMPORTANT: Acest script reprezintă un blueprint de proiectare.
-- NU SE APLICĂ în baza de date în această etapă (Nu aplica SQL).
-- Toate funcțiile sunt definite cu SECURITY DEFINER și SET search_path = public.
-- ############################################################################
...
BEGIN
    ...
    WHERE store_id = p_store_id
      AND created_at >= v_tz_from
      AND created_at <= v_tz_to
      AND status IN ('finalized', 'partially_returned', 'returned');

    -- 3. Anulări (Voids)
    -- Suma sumelor returnate pe tranzacții de anulare finalizate
    SELECT COALESCE(SUM(total_refund), 0), COUNT(*)
    ...
    -- 4. Retururi (Returns)
    -- Suma sumelor returnate pe tranzacții de retur finalizate
    SELECT COALESCE(SUM(total_refund), 0), COUNT(*)
    ...
    -- Preluăm sumele direct din tabela de payments pentru vânzările active
    ...
    WHERE store_id = p_store_id
      AND opened_at >= v_tz_from
      AND opened_at <= v_tz_to
...
-- Afișează performanța produselor pe baza cantităților și profitului net.
-- ============================================================================
CREATE OR REPLACE FUNCTION public.get_product_performance_report(
    ...
    product_name TEXT,
    barcode TEXT,
    quantity_sold_gross NUMERIC,
    quantity_returned NUMERIC,
    quantity_sold_net NUMERIC,
...
BEGIN
    ...
    WITH item_returns AS (
        -- Agregă retururile finalizate per sale_item_id
        SELECT 
        ...
        WHERE sr.store_id = p_store_id
          ...
          -- Filtrăm retururile în intervalul de raportare sau calculate dinamic
    ...
    aggregated_sales AS (
        SELECT 
            ...
            -- Calcul COGS bazat pe costul lotului (stock_batches) sau prețul de achiziție implicit (product_prices)
            SUM(
    ...
    SELECT 
        ...
        p.barcode,
        COALESCE(a.qty_gross, 0)::NUMERIC as quantity_sold_gross,
        COALESCE(a.qty_ret, 0)::NUMERIC as quantity_returned,
        (COALESCE(a.qty_gross, 0) - COALESCE(a.qty_ret, 0))::NUMERIC as quantity_sold_net,
        COALESCE(a.rev_gross, 0)::NUMERIC as gross_revenue,
        COALESCE(a.rev_ret, 0)::NUMERIC as returned_revenue,
        (COALESCE(a.rev_gross, 0) - COALESCE(a.rev_ret, 0))::NUMERIC as net_revenue,
        COALESCE(a.cogs_net, 0)::NUMERIC as estimated_cogs,
        ((COALESCE(a.rev_gross, 0) - COALESCE(a.rev_ret, 0)) - COALESCE(a.cogs_net, 0))::NUMERIC as estimated_profit,
        (
            CASE 
...
BEGIN
    ...
    -- Selectare tură
    SELECT s.*, p.full_name as cashier_name, cr.name as register_name 
    INTO v_shift
    ...
    -- Dacă utilizatorul este simplu casier, poate accesa doar propria tură
    ...
    -- 2. Calcul vânzări brute cash înregistrate în această tură (indiferent de statusul post-vânzare)
    ...
    SELECT COUNT(*) INTO v_transactions_count FROM public.sales WHERE shift_id = p_shift_id AND status IN ('finalized', 'partially_returned', 'returned');
    SELECT COUNT(*) INTO v_voids_count FROM public.sale_returns WHERE shift_id = p_shift_id AND type = 'void' AND status = 'completed';
    SELECT COUNT(*) INTO v_returns_count FROM public.sale_returns WHERE shift_id = p_shift_id AND type = 'return' AND status = 'completed';
...
BEGIN
    ...
    FROM public.stock_batches
    WHERE store_id = p_store_id AND zone = 'depozit';
    ...
    -- Loturi/Produse cu stoc negativ (ar trebui să fie 0)
    SELECT COUNT(*)
    INTO v_negative_stock_count
    FROM public.stock_batches
    WHERE store_id = p_store_id AND quantity < 0;

    -- 5. Candidăți Dead Stock (Slow Movers)
    -- Produse cu stoc pozitiv dar fără nicio vânzare finalizată în ultimele 30 de zile
    SELECT jsonb_agg(
        jsonb_build_object(
            'productId', p.id,
            'name', p.name,
