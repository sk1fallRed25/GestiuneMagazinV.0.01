-- ============================================================================
-- BLUEPRINT SQL: Sales Advanced Returns & Voids (Etapa 6B.3.0 - Pre-Apply Hardening)
-- ============================================================================
-- ATENȚIE: ACEST SCRIPT ESTE UN BLUEPRINT DE PROIECTARE ARHITECTURALĂ RAFINAT.
-- NU SE APLICĂ AUTOMAT ÎN BAZA DE DATE ÎN ETAPA 6B.3.0.
-- ============================================================================

-- 1. INDEXURI SUPLIMENTARE PENTRU OPTIMIZAREA Relațiilor de Retur
-- Recomandate pentru performanță pe chei străine la interogări complexe și reconcilieri
CREATE INDEX IF NOT EXISTS idx_sale_returns_profile ON public.sale_returns(profile_id);
CREATE INDEX IF NOT EXISTS idx_sale_returns_shift ON public.sale_returns(shift_id);
CREATE INDEX IF NOT EXISTS idx_sale_return_items_product ON public.sale_return_items(product_id);
CREATE INDEX IF NOT EXISTS idx_sale_return_items_batch ON public.sale_return_items(batch_id);

-- ============================================================================
-- 2. RPC: get_sale_return_eligibility
-- Consultare eligibilitate și detalii articole pentru retur parțial/total
-- ============================================================================
...
REVOKE ALL ON FUNCTION public.get_sale_return_eligibility(uuid, uuid, uuid) FROM anon;
GRANT EXECUTE ON FUNCTION public.get_sale_return_eligibility(uuid, uuid, uuid) TO authenticated;

-- ============================================================================
-- 3. RPC: return_sale_items
-- Înregistrare tranzacțională a unui retur parțial sau total
-- ============================================================================
...
REVOKE ALL ON FUNCTION public.return_sale_items(uuid, uuid, uuid, jsonb, text, text, text) FROM anon;
GRANT EXECUTE ON FUNCTION public.return_sale_items(uuid, uuid, uuid, jsonb, text, text, text) TO authenticated;

-- ============================================================================
-- 4. PATCH RECONCILIERE TURE (RECONCILIATION AUDIT PATCH)
-- Modificări pentru get_active_pos_shift și close_pos_shift
-- ============================================================================

-- A. get_active_pos_shift
CREATE OR REPLACE FUNCTION public.get_active_pos_shift(p_store_id uuid, p_profile_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
    ...
    v_total_cash_refunds DECIMAL(12,2) := 0;
    v_transactions_count INT := 0;
BEGIN
    -- Găsire tură deschisă
    ...
    -- Calcul vânzări brute active (inclusiv cele returnate parțial/total, pentru a păstra coerența plăților în tura inițială)
    SELECT COALESCE(SUM(total), 0), COUNT(*)
    INTO v_total_sales, v_transactions_count
    FROM public.sales
    WHERE shift_id = v_shift.id AND status IN ('finalized', 'partially_returned', 'returned');

    -- Total plătite prin Cash
    SELECT COALESCE(SUM(p.amount), 0) INTO v_total_cash
    FROM public.payments p
    JOIN public.sales s ON s.id = p.sale_id
    WHERE s.shift_id = v_shift.id AND p.method = 'cash' AND s.status IN ('finalized', 'partially_returned', 'returned');

    -- Total plătite prin Card
    SELECT COALESCE(SUM(p.amount), 0) INTO v_total_card
    FROM public.payments p
    JOIN public.sales s ON s.id = p.sale_id
    WHERE s.shift_id = v_shift.id AND p.method = 'card' AND s.status IN ('finalized', 'partially_returned', 'returned');

    -- Total plătite prin Mixed
    SELECT COALESCE(SUM(total), 0) INTO v_total_mixed
    FROM public.sales
    WHERE shift_id = v_shift.id AND payment_method = 'mixed' AND status IN ('finalized', 'partially_returned', 'returned');

    -- Calcul retururi cash efectuate în tura curentă
    SELECT COALESCE(SUM(total_refund), 0) INTO v_total_cash_refunds
    FROM public.sale_returns
    WHERE shift_id = v_shift.id AND type = 'return' AND status = 'completed' AND refund_method = 'cash';
    ...
    RETURN jsonb_build_object(
        ...
        'current_totals', jsonb_build_object(
            'total_sales', v_total_sales - v_total_cash_refunds, -- Net sales în tură
            'total_cash', v_total_cash,
            'total_card', v_total_card,
            'total_mixed', v_total_mixed,
            'expected_cash', v_shift.opening_cash + v_total_cash - v_total_cash_refunds, -- Sold corect sertar
...
-- B. close_pos_shift
CREATE OR REPLACE FUNCTION public.close_pos_shift(p_store_id uuid, p_profile_id uuid, p_shift_id uuid, p_declared_cash numeric, p_closing_notes text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
    ...
    -- Agregare vânzări brute în tură
    SELECT COALESCE(SUM(total), 0), COUNT(*)
    INTO v_total_sales, v_transactions_count
    FROM public.sales
    WHERE shift_id = p_shift_id AND status IN ('finalized', 'partially_returned', 'returned');

    -- Total plătite cash brute
    SELECT COALESCE(SUM(p.amount), 0) INTO v_total_cash
    FROM public.payments p
    JOIN public.sales s ON s.id = p.sale_id
    WHERE s.shift_id = p_shift_id AND p.method = 'cash' AND s.status IN ('finalized', 'partially_returned', 'returned');

    -- Total plătite card brute
    SELECT COALESCE(SUM(p.amount), 0) INTO v_total_card
    FROM public.payments p
    JOIN public.sales s ON s.id = p.sale_id
    WHERE s.shift_id = p_shift_id AND p.method = 'card' AND s.status IN ('finalized', 'partially_returned', 'returned');

    -- Total plătite mixed brute
    SELECT COALESCE(SUM(total), 0) INTO v_total_mixed
    FROM public.sales
    WHERE shift_id = p_shift_id AND payment_method = 'mixed' AND status IN ('finalized', 'partially_returned', 'returned');

    -- Calcul retururi cash efectuate în această tură
    SELECT COALESCE(SUM(total_refund), 0) INTO v_total_cash_refunds
    FROM public.sale_returns
    WHERE shift_id = p_shift_id AND type = 'return' AND status = 'completed' AND refund_method = 'cash';

    v_expected_cash := v_shift.opening_cash + v_total_cash - v_total_cash_refunds;
    v_cash_diff := p_declared_cash - v_expected_cash;

    -- Închidere tură și stocare valori consolidate
    ...
    SET status = 'closed',
        ...
        total_sales = v_total_sales - v_total_cash_refunds, -- Vânzări nete
        total_cash = v_total_cash - v_total_cash_refunds, -- Cash net obținut
        total_card = v_total_card,
        total_mixed = v_total_mixed,
        transactions_count = v_transactions_count,
    ...
    RETURN jsonb_build_object(
        ...
        'summary', jsonb_build_object(
            'opening_cash', v_shift.opening_cash,
            'total_sales', v_total_sales - v_total_cash_refunds,
            'total_cash', v_total_cash - v_total_cash_refunds,
            'total_card', v_total_card,
            'total_mixed', v_total_mixed,
            'expected_cash', v_expected_cash,
            'declared_cash', p_declared_cash,
