-- ############################################################################
-- BLUEPRINT PROCEDURI STOCATE ATOMICE (RPC) - ETAPA 5D.0
...
-- ============================================================================
-- 1. RPC: finalize_sale
-- Scop: Finalizează o vânzare în mod atomic (creare bon, linii, mișcări, plăți,
--       scădere stoc FEFO/FIFO cu FOR UPDATE).
-- ============================================================================
...
BEGIN
    ...
    SELECT (public.has_store_role(p_store_id, ARRAY['admin', 'casier', 'manager', 'gestionar']) OR public.is_platform_owner()) INTO v_has_role;
    IF NOT v_has_role THEN
        RAISE EXCEPTION 'Acces interzis: Utilizatorul nu are rolul necesar (admin/casier/manager/gestionar/platform_owner) pentru magazinul solicitat.';
    END IF;

    -- 2. Calcul total din DB și validare cantități
    ...
    -- 3. Verificare concordanță plăți vs total calculat
    FOR v_payment IN SELECT * FROM jsonb_array_elements(p_payments)
    LOOP
        v_payment_total := v_payment_total + (v_payment->>'amount')::DECIMAL;
    END LOOP;

    IF v_payment_total <> v_total_calc THEN
    ...
    INSERT INTO public.sales (store_id, profile_id, total_amount, payment_method, status, notes)
    VALUES (p_store_id, p_profile_id, v_total_calc, p_payments->0->>'method', 'completed', p_notes)
    ...
    LOOP
        ...
        LOOP
            ...
            -- Scădere stoc din lot (prevenire stoc negativ)
            UPDATE public.stock_batches
            SET quantity = quantity - v_qty_to_take
            WHERE id = v_batch.id;

            -- Creare sale_items
            INSERT INTO public.sale_items (store_id, sale_id, product_id, quantity, unit_price, total_price)
            VALUES (p_store_id, v_sale_id, v_product_id, v_qty_to_take, v_unit_price, v_qty_to_take * v_unit_price);
    ...
    RETURN jsonb_build_object('sale_id', v_sale_id, 'total_amount', v_total_calc);
END;
...
-- ============================================================================
-- 2. RPC: receive_stock
-- Scop: Recepție marfă în mod atomic (creare recepție, linii, actualizare prețuri,
--       creștere stoc cu FOR UPDATE).
-- ============================================================================
...
DECLARE
    ...
    v_has_role BOOLEAN;
BEGIN
    -- 1. Validare permisiuni (admin, gestionar, manager, platform_owner)
    SELECT (public.has_store_role(p_store_id, ARRAY['admin', 'gestionar', 'manager']) OR public.is_platform_owner()) INTO v_has_role;
    IF NOT v_has_role THEN
        RAISE EXCEPTION 'Acces interzis: Utilizatorul nu are rolul necesar (admin/gestionar/manager/platform_owner).';
    END IF;
    ...
    LOOP
        ...
        v_vat_percent := (v_item->>'vat_percent')::DECIMAL;
...
REVOKE EXECUTE ON FUNCTION public.receive_stock(UUID, UUID, TEXT, TEXT, TEXT, TEXT, TEXT, JSONB) FROM anon;
GRANT EXECUTE ON FUNCTION public.receive_stock(UUID, UUID, TEXT, TEXT, TEXT, TEXT, TEXT, JSONB) TO authenticated;


-- ============================================================================
-- 3. RPC: transfer_stock
-- Scop: Transfer atomic între zone (depozit <-> magazin) cu respectarea
--       FEFO/FIFO și blocare loturi sursă/țintă cu FOR UPDATE.
-- ============================================================================
...
-- ============================================================================
-- 4. RPC: record_waste
-- Scop: Înregistrare pierdere/casare în mod atomic cu deducere din loturi
--       FEFO/FIFO și blocare cu FOR UPDATE.
-- ============================================================================
...
BEGIN
    ...
    -- 3. Procesare loturi (FEFO/FIFO cu FOR UPDATE)
    -- Dacă p_source_zone este 'auto', consumăm din magazin, apoi din depozit
    FOR v_batch IN 
    ...
    IF v_rem_qty > 0 THEN
        RAISE EXCEPTION 'Stoc insuficient pentru casare (Rămas neacoperit: %).', v_rem_qty;
    END IF;
