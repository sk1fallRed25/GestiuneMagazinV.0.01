...
def run_teardown(browser, port="5173"):
    safe_print("\n--- 6. TEARDOWN DATE DB (via admin@owner.com) ---")
    try:
        ...
        teardown_res = page_td.evaluate("""async () => {
            ...
            if (e2eStoreIds.length > 0) {
                ...
                // 2. Stergem audit_logs pentru aceste magazine daca exista tabelul
                try {
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            try:
                ...
                page2.goto(f"http://localhost:{port}/#/setari-magazin")
                page2.wait_for_load_state("networkidle")
            ...
            page5.goto(f"http://localhost:{port}/#/login")
